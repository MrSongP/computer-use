...
export class SetValueHandler {
  readonly definition = setValueCapability;
  ...
  async handle(request: JsonRpcRequest<SetValueParams>): Promise<JsonRpcResponse<null>> {
    return this.context.trace.runAction({
      ...
      execute: async (trace) => {
        ...
        const params = validateSetValueParams(request.params);
        trace.setInputParams({ ...params, value: `<text:${params.value.length} chars>` });
        ...
        const execution = await service.setValue(params);
