...
3. `doc/computer-use.md` for architecture and capability boundaries.
4. `.agents/computer-use-harness.md` for tool semantics and lifecycle rules.
5. `.agents/computer-use-maintenance-checklist.md` for completion requirements.
...
| Windows capture, input, UIA, activation, discovery, or native host | `doc/windows_native_interface/windows-native-interface-design.md`, the relevant TypeScript Windows service, native-host implementation, and tests |
...
5. Update user-facing and agent-facing documentation in the same change.
...
- Human-facing documentation belongs in `README*.md` and `doc/`.
- Agent operating contracts belong in `AGENTS.md`, `.agents/`, `.claude/`, and the plugin skill.
- Keep `.agents/` and `.claude/` mirror documents synchronized where both exist.
